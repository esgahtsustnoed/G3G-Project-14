...
# overview.to_csv('../output/no_alcohol/data_overview.csv', index=False)
...
table_grade_zero = pd.DataFrame(columns=['G1', 'G2', 'G3'])
for index, row in data_whole.iterrows():
    if row['G1'] == 0 or row['G2'] == 0 or row['G3'] == 0:
        print(row['G1'], row['G2'], row['G3'])
        table_grade_zero.loc[index] = [row['G1'], row['G2'], row['G3']]
print(table_grade_zero)
table_grade_zero.to_csv('../output/alcohol/Grade Zero.csv')
# table_grade_zero.to_csv('../output/no_alcohol/Grade Zero.csv')
...
# descr.to_csv(f'../output/no_alcohol/Descriptive Statistics before cleaning.csv')
...
for i, feature in enumerate(data_whole.select_dtypes(include='int64').columns):
    ...
    axes[0][col].text(0.98, 0.97, f"Upper whisker: {upper_whisker:.2f}", transform=axes[0][col].transAxes,
                      verticalalignment='top', horizontalalignment='right', bbox=dict(facecolor='white', alpha=0.5))
    axes[1][col].text(0.98, 0.97, f"Upper whisker: {upper_whisker:.2f}", transform=axes[1][col].transAxes,
                      verticalalignment='top', horizontalalignment='right', bbox=dict(facecolor='white', alpha=0.5))
...
# fig.savefig('../output/no_alcohol/Boxplot outliers vs no outliers.pdf')
plt.show()
...
# descr_after.to_csv(f'../output/no_alcohol/Descriptive Statistics after cleaning.csv')
...
for i, feature in enumerate(data_whole.select_dtypes(include=['int64']).columns):
    ax = axes[i]
    sns.histplot(data=data_whole, x=feature, ax=ax, binwidth=0.99999, color='#969696')
    ax.set_title(feature + ' histogram')
    ax.set_xticks(range(data_whole[feature].min(), data_whole[feature].max() + 1))

    ax.set_xlim([data_whole[feature].min(), data_whole[feature].max()])

fig.tight_layout()
fig.savefig("../output/alcohol/Distribution histograms.pdf")
# fig.savefig("../output/no_alcohol/Distribution histograms.pdf")
plt.show()
...
for column in data_whole.select_dtypes(include='int64').columns:
    ...
    plt.savefig(f"../output/alcohol/QQ plot of {column}.pdf")
    # plt.savefig(f"../output/no_alcohol/QQ plot of {column}.pdf")
    plt.show()
...
for k in range(1, X.shape[1] + 1):
    ...
    scores = cross_val_score(model, X_train_UVFS, y_train, cv=5, scoring='r2')
...
print('Amount of selected features: ', len(selected_features_UVFS))
...
print('Amount of selected features: ', len(selected_features_L1))
...
# UVFS plot
amount_selected_features_UVFS = len(selected_features_UVFS)
plt.figure(figsize=(12, 8))
plt.bar(range(len(sorted_scores_UVFS[:amount_selected_features_UVFS])), sorted_scores_UVFS[:amount_selected_features_UVFS], width=0.9, color='#bdbdbd')
plt.xticks(range(len(sorted_features_UVFS[:amount_selected_features_UVFS])), sorted_features_UVFS[:amount_selected_features_UVFS], rotation=90, fontsize=14)
plt.xlabel('Feature')
plt.ylabel('Statistical Significance of Features')
plt.yticks(fontsize=14)
plt.title('Selected Features Based on Univariate Analysis')
plt.tight_layout()
plt.xlim(-0.5, amount_selected_features_UVFS-0.5)  # Limits the x-axis to the first 28 features
plt.savefig(f"../output/alcohol/Selected Features UVFS.pdf")
# plt.savefig(f"../output/no_alcohol/Selected Features UVFS.pdf")
plt.show()

# L1 plot
amount_selected_features_L1 = len(selected_features_L1)
plt.figure(figsize=(12, 8))
plt.bar(range(len(sorted_scores_L1[:amount_selected_features_L1])), sorted_scores_L1[:amount_selected_features_L1], width=0.9, color='#bdbdbd')
plt.xticks(range(len(sorted_features_L1[:amount_selected_features_L1])), sorted_features_L1[:amount_selected_features_L1], rotation=90, fontsize=14)
plt.xlabel('Feature')
plt.ylabel('L1 Regularization Coefficient Magnitude')
plt.yticks(fontsize=14)
plt.title('Selected Features Based on L1 Analysis')
plt.tight_layout()
plt.xlim(-0.5, amount_selected_features_L1-0.5)
plt.savefig(f"../output/alcohol/Selected Features L1.pdf")
# plt.savefig(f"../output/no_alcohol/Selected Features L1.pdf")
plt.show()
...
# mean_std_performance.to_csv(f'../output/no_alcohol/CV_Mean_Performance.csv')
...
# Data frame to save the evaluation metrics
df_top_performance = pd.DataFrame(columns=['Train/Test', 'R2', 'MSE', 'RMSE', 'MAE'])
...
# df_top_performance.to_csv(f'../output/no_alcohol/Best_performance_{best_model_validation}_{best_feature_selection}.csv', index=False)
...
def alcohol_boxplot(data, a, b, day):
    ...
    plt.savefig(f'../output/alcohol/Mean Grade vs alcohol boxplot_{b}.pdf')
    plt.close()
